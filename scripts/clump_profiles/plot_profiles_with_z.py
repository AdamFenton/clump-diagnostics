...
bins = np.logspace(np.log10(3e-4),np.log10(50),100) # change the number of bins ?
...
def spherical_average(subsnap,bins):
    ...
    mass_in_bin = np.cumsum(calculate_number_in_bin(R,subsnap['m'])[0]) * subsnap['m'][0].to('jupiter_mass')
    ...
    return avg_infall,avg_rotational,avg_density,avg_temperature, R,interp_start,interp_end, mass_in_bin
...
def axisym_average(subsnap,clump_centre,clump_velocity,bins,smoothing_factor):
    ...
    for part in range(n_part):
        for bin in range(len(bins)-1):
            if R[part] < bins[bin+1] and R[part] > bins[bin] \
            and np.abs(z[part]) < smoothing_factor*(bins[bin+1] - bins[bin]) :
                particles_ids[bin].append(part)
                bin_counter[bin] += 1
                infall_in_bin[bin] += infall[part].magnitude
                rotational_in_bin[bin] += rotational[part].magnitude
...
axi_comp,z_comp,clump_centre,clump_velocity,full_clump = prepare_snapshots('%s' % sys.argv[1])
...
axi,interp_start_axi,interp_end_axi = axisym_average(axi_comp,
                                                           clump_centre,
...
avg_infall_sphere, avg_rotational_sphere, avg_density_sphere, avg_temperature_sphere, \
                                    R_sphere,interp_start_sphere,interp_end_sphere, mass = spherical_average(full_clump,bins)
...
for index,label,limit in zip(figure_indexes,figure_ylabels,figure_ylimits):
    ...
    ax_main[index].set_xlim(1e-4,50)
for i in range(3):
    for j in range(2):
...
R_axi_peaks,R_axi_mins = find_infall_peaks(avg_infall_axi,interp_start_axi,interp_end_axi)[1:]
...
ax_main[0,0].plot(R_axi,avg_density_axi,c='black',linestyle='-')
ax_main[0,0].scatter(r_tmp,axi_comp['density'],s=0.1)
...
ax_main[0,1].plot(R_axi,avg_temperature_axi,c='black',linestyle='-')
...
ax_main[1,0].plot(R_axi,avg_rotational_axi,c='black',linestyle='-')
#------------------------------------------------------------------------------#
#------------------------------------------------------------------------------#
...
ax_main[1,1].plot(bins,avg_infall_z_pos,c='blue')
ax_main[1,1].plot(R_axi,avg_infall_axi,c='black',linestyle='-')
#------------------------------------------------------------------------------#

#------------------------------------------------------------------------------#
ax_main[2,0].plot(bins[1:],mass,c='black')
#------------------------------------------------------------------------------#

#------------------------------------------------------------------------------#
ax_main[2,1].plot(bins[interp_start_z_pos:interp_end_z_pos],interpolate_across_nans(avg_infall_z_pos)[interp_start_z_pos:interp_end_z_pos],c='blue',linestyle='-',linewidth=0.75,alpha=0.5)
ax_main[2,1].plot(bins,avg_infall_z_pos,c='blue',label='z direction')
ax_main[2,1].plot(R_axi,avg_infall_axi,c='black',linestyle='-',label='Axisymmetric average')
ax_main[2,1].legend(loc='upper center', bbox_to_anchor=(-0.3, -0.3),
